...
class BasePage:
    ...
    def is_element_present(self, method, selector):
        try:
            self.browser.find_element(by=method, value=selector)
        except NoSuchElementException:
            return False
        return True

    def is_disappeared(self, selector, method, timeout=4):
        try:
            WebDriverWait(self.browser, timeout, 1, TimeoutException). \
                until_not(EC.presence_of_element_located((selector, method)))
